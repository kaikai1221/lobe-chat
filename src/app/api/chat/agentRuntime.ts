...
class AgentRuntime {
  ...
  async chat(
    ...
    const generation = trace?.generation({
      input: messages,
      metadata: { provider },
      model: tools ? 'gpt-3.5-turbo-16k' : model,
  ...
  static async initializeWithUserPayload(provider: string, payload: JWTPayload, isTools?: boolean) {
    ...
    switch (type) {
      ...
      case ModelProvider.OpenAI: {
        runtimeModel = this.initOpenAI(payload, isTools);
        break;
      }
  ...
  private static initOpenAI(payload: JWTPayload, isTools?: boolean) {
